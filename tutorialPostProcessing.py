...
def findCoordinationNumber(points_array):
    ...
    for i in range(points_array.shape[0]):
        for index in range(i, points_array.shape[0]):
            pointDist = distanceBetweenPoints(points_array[i], points_array[index])
            if pointDist != 0 and pointDist <= neighborThreshold:
                totalNeighbors += 1
    return totalNeighbors*2/points_array.shape[0]
...
def getAggregateValuesList(aggregateNumber, fracNumber):
    aggregateValuesList = list(range(500))
    for files in particles_by_agg_frac[(f"aggregate_{aggregateNumber}", f"frac_{fracNumber}")]:
        ...
        aggregateValuesList[timestep] = (findRadiusOfGyration(pointsArr), findCoordinationNumber(pointsArr))
